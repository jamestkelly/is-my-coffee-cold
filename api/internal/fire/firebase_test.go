...
func TestGetFirebaseAdminConfiguration(t *testing.T) {
	...
	if err != nil {
		t.Errorf("Unable to set `FIREBASE_TYPE` environment variable due to error: %v.", err)
	}

	err = os.Setenv("FIREBASE_PROJECT_ID", expectedValue)
	if err != nil {
		t.Errorf("Unable to set `FIREBASE_PROJECT_ID` environment variable due to error: %v.", err)
	}

	err = os.Setenv("FIREBASE_PRIVATE_KEY_ID", expectedValue)
	if err != nil {
		t.Errorf("Unable to set `FIREBASE_PRIVATE_KEY_ID` environment variable due to error: %v.", err)
	}

	err = os.Setenv("FIREBASE_PRIVATE_KEY", expectedValue)
	if err != nil {
		t.Errorf("Unable to set `FIREBASE_PRIVATE_KEY` environment variable due to error: %v.", err)
	}

	err = os.Setenv("FIREBASE_CLIENT_EMAIL", expectedValue)
	if err != nil {
		t.Errorf("Unable to set `FIREBASE_CLIENT_EMAIL` environment variable due to error: %v.", err)
	}

	err = os.Setenv("FIREBASE_CLIENT_ID", expectedValue)
	if err != nil {
		t.Errorf("Unable to set `FIREBASE_CLIENT_ID` environment variable due to error: %v.", err)
	}

	err = os.Setenv("FIREBASE_AUTH_URI", expectedValue)
	if err != nil {
		t.Errorf("Unable to set `FIREBASE_AUTH_URI` environment variable due to error: %v.", err)
	}

	err = os.Setenv("FIREBASE_TOKEN_URI", expectedValue)
	if err != nil {
		t.Errorf("Unable to set `FIREBASE_TOKEN_URI` environment variable due to error: %v.", err)
	}

	err = os.Setenv("FIREBASE_AUTH_PROVIDER_X509_CERT_URL", expectedValue)
	if err != nil {
		t.Errorf("Unable to set `FIREBASE_AUTH_PROVIDER_X509_CERT_URL` environment variable due to error: %v.", err)
	}

	err = os.Setenv("FIREBASE_CLIENT_X509_CERT_URL", expectedValue)
	if err != nil {
		t.Errorf("Unable to set `FIREBASE_CLIENT_X509_CERT_URL` environment variable due to error: %v.", err)
	}

	err = os.Setenv("FIREBASE_UNIVERSE_DOMAIN", expectedValue)
	if err != nil {
		t.Errorf("Unable to set `FIREBASE_UNIVERSE_DOMAIN` environment variable due to error: %v.", err)
	}
